...
void mozilla_sampler_add_marker(const char *aMarker, ProfilerMarkerPayload *aPayload)
{
  ...
  stack->addMarker(aMarker, payload.release(), static_cast<float>(delta.ToMilliseconds()));
}
...
::Mutex* OS::CreateMutex(const char* aDesc) {
  return new GeckoMutex(aDesc);
}
